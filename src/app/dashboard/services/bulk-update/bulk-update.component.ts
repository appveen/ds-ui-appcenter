...
export class BulkUpdateComponent implements OnInit, OnDestroy, CanComponentDeactivate {
  ...
  @ViewChild('partialSuccess', { static: false }) partialSuccess: TemplateRef<HTMLElement>;
  ...
  buildForm(data, value?) {
    ...
    if (self.isEdit && !self.hasPermission('PUT')) {
      return self.router.navigate(['/~/services', self.schema._id, 'list']);
    }
  ...
  submitValue(reset?: boolean, draft?: boolean) {
    ...
    self.showLazyLoader = true;
    self.subscriptions['saveRecord'] = self.commonService.put('api', url, payload).subscribe(res => {
      const arr = [];
      if (res._workflow) {
        ...
        if (res && res.every(e => e._id)) {
          ...
          self.partialSuccessRef.result.then(close => {
            self.resolveAll(reset, draft, res, arr);
          }, dismiss => {
            self.resolveAll(reset, draft, res, arr);
          });
  ...
  submitWorkflowFiles(wfId, reset, isDraft?) {
    ...
    return self.commonService.put('wf', '/' + wfId, payload).toPromise();
  }
