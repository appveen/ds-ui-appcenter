...
export class InteractionsAllComponent implements OnInit, OnDestroy {
      ...
      getRows: async (params: IGetRowsParams) => {
        try {
          ...
          if (count > 0) {
            ...
            self.subscription['getRecords_' + self.apiConfig.page] = self.getInteraction().subscribe(docs => {
  ...
  rowDoubleClicked(event: RowDoubleClickedEvent) {
    ...
    self.router.navigate([`~/interactions/${event.data.partnerId}/${event.data.flowId}/${event.data.odpTxnId}`]);
  }
